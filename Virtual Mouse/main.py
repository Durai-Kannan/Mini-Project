...
def main():
    ...
    try:
        while cap.isOpened():
            ...
            # Brightness control and scrolling with left hand
            if left_landmark_list:
                # Scrolling control (only if index and middle fingers are close together)
                index_finger_tip_left = left_landmark_list[1]  # Left hand index finger tip
                middle_finger_tip_left = left_landmark_list[2]  # Left hand middle finger tip

                # Check if index and middle fingers are together
                finger_distance_left = get_distance_between_fingers(frame, index_finger_tip_left, middle_finger_tip_left)

                if finger_distance_left < 40:  # Fingers close enough to enable scrolling
                    scroll_position = np.interp(index_finger_tip_left[1], [0, frame.shape[0]], [-75, 75])
                    pyautogui.scroll(int(scroll_position))
                    cv2.putText(frame, f'Scroll: {int(scroll_position)}', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
